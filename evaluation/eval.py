...
for each in tt:
    ...
    def work(model_out):
        ...
        if isinstance(model_out['model'], list):
            model_out['model'] = model_out['model'][0]
        r, p, f = score([
            model_out["model"],
        ], [model_out['src']['conversation'][-1]['doctor']], lang='zh')
        br += r[0]
        bf += p[0]
        bp += f[0]
        b_div += 1
        logger.info(model_out)
        if is_ask(model_out["model"]):
            ...
            if get_is_informational(
                model_out['src']['conversation'][-1]['patient'], 
                model_out["model"], 
                actual_resp, 
                model_out['src']['conversation'][:-1]):
                informational += 1
                is_f = True

# a pickle file that contains document object for BM25Retriever
# retriver is used to do fact check
# the Docuemtn should be in format of 
# Document(
#  page_content="some page content relevent"
# )
corpus_path = ""
# some of the allowed keywords, only these keywords are allowed to be used in the retrieval stage
...
retriver = BM25Retriever.from_documents(corpus, k=8)
...
def get_auv(model_out, objs: MedicalAskContent):
    conversation = model_out[0]['conversation']
    conv_id = cal_hash(str(conversation[0]))
    conv = conv_id_to_c[conv_id]
    cur_round = len(model_out[0]['conversation'])
    stacked_patients = "\n".join([
...
for each in tt:
    ...
    # data = json.load(open("./input/{}".format(each)))
    data = pickle.load(open("./input-pkl/{}".format(each), "rb"))
    ...
    def work(model_out):
        ...
        model_out_is_ask = isinstance(
            model_out[2], AskAction
        )
        model_resp = model_out[2]
        try:
            model_resp = model_resp.text
        except:
            model_resp = ""
        r, p, f = score([
            model_resp,
        ], [model_out[0]['conversation'][-1]['doctor']], lang='zh')
        br += r[0]
        bf += p[0]
        bp += f[0]
        b_div += 1
        ...
        if model_out_is_ask:
            ...
            actual_resp = model_out[0]['conversation'][-1]['doctor']
            actual_entities = generate_entities(actual_resp)
            t_auv = get_auv(model_out, model_entities)
            ...
            actual_resp = model_out[0]['conversation'][-1]['doctor']
            is_f = False
            if get_is_informational(
                model_out[0]['conversation'][-1]['patient'], 
                model_resp, 
                actual_resp, 
                model_out[0]['conversation'][:-1]):
                informational += 1
                is_f = True
            total_informational_div += 1
            if is_f:
                if not hallucinate(
                    model_out[0]['conversation'][-1]['patient'], 
                    model_resp, 
                    actual_resp, 
                    model_out[0]['conversation'][:-1]):
                    support_info.append(1)
                else:
                    support_info.append(0)
    futures = []
    for i in tqdm(data[:]):
        work(i)
    with open("./output/{}.txt".format(each), "w") as f:
        f.write("\n")
        f.write(f"ask: {ask}\ntell: {tell}\n")
        import numpy as np
        f.write(f"lev distance: {np.mean(lev_info)}\n")
        f.write(f"""auv presence: {np.sum(
            [e[0] for e in auv]
        )}\n""")
        f.write(f"""auv non presence: {np.sum(
            [e[1] for e in auv]
        )}\n""")
